...
export const getMovieAPI = async (movie_id: number) => {
  const URL = `${API_URL}/movie/${movie_id}?api_key=${API_TOKEN}&append_to_response=videos`
  const data = await fetch(URL).then((response) => response.json())
  return data
}

export const getMoviesAPI = async ({ page = 1, query = '' }: { page: number | null, query: string | null }) => {
...
export const discoverMoviesAPI = async ({ page = 1 }: { page: number }) => {
